...
npm run build -w reactjs-todo-login-widget # if in workspace root

npm run build # if in the reactjs-todo-login-widget folder
```
...
npm run start:reactjs-todo-lw # only if in root

npm start # if in reactjs-todo-login-widget folder
```
...
This project has debugging statements that can be activated which causes the app to pause execution at each Login Widget integration point. It will have a comment above the `debugger` statement explaining the purpose of the integration.
...
This sample application does not utilize TypeScript, but if you'd like to see a version of this written in TypeScript, let us know.
