...
export default function ConfigurationScreen(props: Props): React.ReactElement {
  const { profiles, selectedJourneyProfileKey, onSelectJourneyProfile } =
    props;
  ...
  return (
    ...
    >
      <View style={commonStyles.configSection}>
        <Text style={commonStyles.configSectionTitle}>Journey</Text>

        {profiles.map(profile => {
          const selected = profile.key === selectedJourneyProfileKey;
          const iconName = selected ? 'check' : 'check-box-outline-blank';
          return (
            <TouchableOpacity
              key={profile.key}
              style={commonStyles.configOptionRow}
              onPress={() => onSelectJourneyProfile(profile.key)}
