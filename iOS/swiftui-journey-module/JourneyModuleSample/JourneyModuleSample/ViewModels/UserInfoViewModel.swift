...
class UserInfoViewModel: ObservableObject {
    ...
    func fetchUserInfo() async {
        ...
        let userInfo: Result<UserInfo, OidcError>? = await journeyUser?.userinfo(cache: false)
        ...
        case .none:
            ...
            await MainActor.run {
                self.userInfo = "No active session. Please start the Journey flow to authenticate."
            }
        }
    }
}
